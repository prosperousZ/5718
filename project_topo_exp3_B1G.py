...
def run_experiment_3(net):
    "Experiment 3 (delay topology): TCP, UDP, ICMP between h2 and h1."
    h1 = net.get('h1')
    h2 = net.get('h2')
    ...
    h2.cmd('pkill iperf'); h2.cmd('pkill ping')

    # ===== TCP + ping (RTT/loss during TCP flow, under delay topology) =====
    print("\n=== Experiment 3 (delay): TCP h2 -> h1 (with concurrent ping) ===")
    h1.cmd('iperf -s &')
    h2.cmd(f'ping -i 0.2 -c 50 {server_ip} > exp3_ping_during_tcp_h2_h1.log &')
    ...
    print(tcp_output)
    with open('exp3_tcp_h2_h1.log', 'w') as f:
        f.write(tcp_output)
    print("Saved TCP log to exp3_tcp_h2_h1.log")
    print("Saved ping-during-TCP log to exp3_ping_during_tcp_h2_h1.log")

    # ===== UDP + ping (RTT/loss during UDP flow, under delay topology) =====
    print("\n=== Experiment 3 (delay): UDP h2 -> h1 (with concurrent ping) ===")
    h1.cmd('pkill iperf'); h2.cmd('pkill iperf'); h2.cmd('pkill ping')
    h1.cmd('iperf -s -u &')

    h2.cmd(f'ping -i 0.2 -c 50 {server_ip} > exp3_ping_during_udp_h2_h1.log &')
    ...
    print(udp_output)
    with open('exp3_udp_h2_h1.log', 'w') as f:
        f.write(udp_output)
    print("Saved UDP log to exp3_udp_h2_h1.log")
    print("Saved ping-during-UDP log to exp3_ping_during_udp_h2_h1.log")

    # ===== 纯 ICMP baseline under delay topology =====
    print("\n=== Experiment 3 (delay): ICMP ping-only h2 -> h1 ===")
    ping_output = h2.cmd(f'ping -c 20 {server_ip}')
    print("--- Ping-only raw output (exp3) ---")
    print(ping_output)
    with open('exp3_ping_h2_h1.log', 'w') as f:
        f.write(ping_output)
    print("Saved ping-only log to exp3_ping_h2_h1.log")
